<template>
  <div class="p-6 lg:p-10 max-w-7xl mx-auto min-h-screen bg-[#FDFDFD] space-y-10 animate-in fade-in duration-700">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
      <div class="space-y-4">
        <div class="space-y-1">
          <p class="text-[10px] font-black text-orange-500 uppercase tracking-[0.3em]">Personnel Management</p>
      ...
      <button @click="showCreateModal = true" class="px-8 py-4 bg-slate-900 text-white rounded-lg font-black text-xs uppercase tracking-widest hover:bg-orange-500 transition-all shadow-xl shadow-orange-100 active:scale-95 flex items-center">
    ...
    <div class="flex flex-col md:flex-row gap-4">
      <div class="relative flex-1 group">
        ...
        <input 
          ...
          class="w-full pl-12 pr-6 py-4 bg-white border border-slate-100 rounded-xl text-sm font-medium focus:ring-4 focus:ring-orange-50 outline-none transition-all shadow-sm"
        />
      </div>
      <select v-model="filterStatus" class="px-6 py-4 bg-white border border-slate-100 rounded-xl text-xs font-black uppercase tracking-widest text-slate-500 outline-none shadow-sm">
    ...
    <div v-if="loading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      <!-- Skeleton Loader -->
      <div v-for="i in 6" :key="i" class="bg-slate-50 h-64 rounded-xl animate-pulse"></div>
    </div>

    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      <!-- Empty State -->
      <div v-if="filteredUsers.length === 0" class="col-span-full py-32 bg-slate-50 border border-dashed border-slate-200 rounded-xl text-center space-y-4">
        <div class="w-16 h-16 bg-white rounded-xl flex items-center justify-center mx-auto shadow-sm">
      ...
      <div v-for="user in filteredUsers" :key="user.User_ID" 
        class="bg-white p-8 rounded-xl border border-slate-100 shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 group relative overflow-hidden">
        
        <!-- Header: Avatar & Menu -->
        <div class="flex items-start justify-between mb-8">
          <div class="w-16 h-16 bg-slate-900 text-white rounded-lg flex items-center justify-center font-black text-2xl shadow-lg shadow-slate-200 group-hover:rotate-6 transition-transform">
          ...
          <div class="flex flex-col items-end space-y-2">
            ...
            <button class="p-2 text-slate-300 hover:text-orange-500 transition-colors">
        ...
        <!-- Body -->
        <div class="space-y-1">
          <h3 class="text-xl font-black text-slate-900 tracking-tight group-hover:text-orange-500 transition-colors">
        ...
        <div class="mt-8 pt-6 border-t border-slate-50 flex justify-between items-center">
          <div class="space-y-1">
            ...
            <p class="text-[10px] font-black text-orange-500 uppercase tracking-tight">{{ user.jobTitle || 'Staff' }}</p>
          </div>
        ...
        <div class="absolute -bottom-12 -right-12 w-24 h-24 bg-orange-50 rounded-full opacity-0 group-hover:opacity-100 transition-opacity blur-2xl"></div>
      </div>
    </div>
    ...
    <Transition name="modal">
      <div v-if="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center p-6">
        ...
        <div class="bg-white rounded-xl p-12 max-w-xl w-full shadow-2xl relative z-10 animate-in zoom-in duration-300">
