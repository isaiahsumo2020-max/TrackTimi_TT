                ...
                <tr v-for="user in dept.users" :key="user.id" class="group hover:bg-slate-50 transition-colors">
                  <td class="py-4 px-2">
                    <div class="flex items-center space-x-3">
                      <div v-if="user.avatar" class="w-8 h-8 bg-slate-900 text-white rounded-lg overflow-hidden flex-shrink-0">
                        <img :src="`data:${user.avatarMimeType};base64,${user.avatar}`" :alt="user.name" class="w-full h-full object-cover" />
                      </div>
                      <div v-else class="w-8 h-8 bg-slate-900 text-white rounded-lg flex items-center justify-center font-bold text-[10px]">
                        {{ user.name[0] }}
                      </div>
