    ...
    <div class="w-full max-w-3xl relative z-10 animate-in fade-in zoom-in duration-700">
      ...
      <div class="text-center mb-12 space-y-3">
        <router-link to="/" class="inline-flex items-center space-x-3 group mb-4">
          <div class="w-12 h-12 rounded-xl bg-orange-500 flex items-center justify-center shadow-xl shadow-orange-200 group-hover:rotate-6 transition-transform">
            <ZapIcon class="w-7 h-7 text-white fill-white" />
          </div>
          <span class="text-2xl font-black tracking-tighter text-slate-900">TrackTimi<span class="text-orange-500">.</span></span>
        </router-link>
      ...
      <!-- Registration Card -->
      <div class="bg-white rounded-xl border border-slate-100 p-10 md:p-16 shadow-2xl shadow-orange-100/50">
        <form @submit.prevent="handleRegister" class="space-y-12">
          
          <!-- Section 1: Identity Matrix -->
          <div class="space-y-8">
            <div class="flex items-center space-x-3 border-l-4 border-orange-500 pl-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-1.5">
                ...
                <input v-model="form.orgName" type="text" required placeholder="e.g. Acme Corp"
                  class="w-full px-6 py-4 rounded-xl bg-slate-50 border-none focus:ring-2 focus:ring-orange-500 transition-all text-sm font-bold text-slate-900" />
              </div>
              ...
              <div class="space-y-1.5">
                ...
                <input v-model="form.orgDomain" type="text" required placeholder="acme.tracktimi.com" @input="onDomainInput"
                  class="w-full px-6 py-4 rounded-xl bg-slate-50 border-none focus:ring-2 focus:ring-orange-500 transition-all text-sm font-bold text-orange-500 font-mono" />
              </div>
              ...
              <div class="space-y-1.5">
                ...
                <select v-model="form.orgTypeId" required
                  class="w-full px-6 py-4 rounded-xl bg-slate-50 border-none focus:ring-2 focus:ring-orange-500 transition-all text-sm font-bold text-slate-700">
            ...
            <div class="space-y-4">
              <div class="flex justify-between items-center px-2">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center">
                  <MapPinIcon class="w-3 h-3 mr-2 text-orange-500" /> HQ Geolocation
                </label>
                <button type="button" @click="refreshLocation" :disabled="locationLoading"
                  class="text-[9px] font-black text-orange-500 uppercase bg-orange-50 px-3 py-1 rounded-lg hover:bg-orange-100 transition-colors disabled:opacity-50">
                  {{ locationLoading ? 'Syncing...' : '📡 Re-Sync GPS' }}
                </button>
              </div>
              
              <div class="p-6 bg-slate-950 rounded-xl border border-slate-800 shadow-inner relative overflow-hidden group">
                <div class="relative z-10 flex justify-between items-center">
                  <div class="space-y-1">
                    <p class="text-[10px] font-black text-orange-400 uppercase tracking-tighter">{{ locationStatus }}</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10 pt-4">
              <div class="space-y-4">
                ...
                <div class="flex items-center space-x-4">
                  <div v-if="logoPreview" class="w-16 h-16 rounded-xl border-2 border-slate-100 overflow-hidden shadow-md">
                    <img :src="logoPreview" class="w-full h-full object-cover" />
                  </div>
                  <div v-else class="w-16 h-16 rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 flex items-center justify-center">
                    <ImageIcon class="w-6 h-6 text-slate-300" />
                  </div>
                  <input type="file" accept="image/*" @change="handleLogoUpload" 
                    class="flex-1 text-[10px] text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-[10px] file:font-black file:uppercase file:bg-slate-100 file:text-slate-600 hover:file:bg-orange-50 hover:file:text-orange-500 cursor-pointer" />
                </div>
              </div>
          ...
          <div class="space-y-8 pt-10 border-t border-slate-100">
            <div class="flex items-center space-x-3 border-l-4 border-orange-500 pl-4">
              <ShieldCheckIcon class="w-5 h-5 text-orange-500" />
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-1.5">
                ...
                <input v-model="form.firstName" type="text" required placeholder="John"
                  class="w-full px-6 py-4 rounded-xl bg-slate-50 border-none focus:ring-2 focus:ring-orange-500 text-sm font-bold" />
              </div>
              <div class="space-y-1.5">
          ...
          <!-- Consent & Submit -->
          <div class="space-y-8">
            <label class="flex items-start space-x-4 p-6 bg-slate-50 rounded-lg border border-slate-100 cursor-pointer hover:bg-orange-50/50 transition-colors group">
              <input v-model="form.agreeToTerms" type="checkbox" required
                class="mt-1 w-5 h-5 rounded border-slate-300 text-orange-500 focus:ring-orange-500 transition-all cursor-pointer" />
              <span class="text-[11px] font-bold text-slate-500 leading-relaxed uppercase tracking-wider group-hover:text-slate-700">
                ...
                <a href="#" class="text-orange-500 underline">Operational Protocols</a> and 
                <a href="#" class="text-orange-500 underline">Privacy Matrix</a>.
              </span>
            </label>

            <div v-if="errorMessage" class="p-4 bg-red-50 border border-red-100 text-red-600 text-[10px] font-black uppercase tracking-widest rounded-xl text-center animate-pulse">
              ⚠️ {{ errorMessage }}
            </div>

            <button type="submit" :disabled="isSubmitting || !form.agreeToTerms || !isFormValid"
              class="w-full py-6 bg-slate-900 hover:bg-orange-500 text-white rounded-xl font-black text-xs uppercase tracking-[0.3em] shadow-2xl shadow-orange-100 transition-all active:scale-[0.98] disabled:opacity-40 flex items-center justify-center">
      ...
      <p class="mt-12 text-center text-[11px] font-bold text-slate-400 uppercase tracking-widest">
        Existing Authority?
        <router-link to="/login" class="text-orange-500 hover:text-orange-600 ml-1">Access Terminal</router-link>
      </p>
    </div>
  </div>
</template>
