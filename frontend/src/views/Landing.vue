<template>
  <div class="min-h-screen bg-[#FDFDFD] text-slate-900 font-sans selection:bg-orange-100 selection:text-orange-600">
    ...
    <nav class="fixed top-0 left-0 right-0 z-50 bg-white/70 backdrop-blur-xl border-b border-slate-100">
      <div class="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        <div class="flex items-center space-x-3 group cursor-pointer">
          <div class="w-10 h-10 rounded-xl bg-orange-500 flex items-center justify-center shadow-lg shadow-orange-200 group-hover:rotate-6 transition-transform">
            <ZapIcon class="w-6 h-6 text-white fill-white" />
          </div>
          <span class="font-black text-2xl tracking-tighter text-slate-900">TrackTimi<span class="text-orange-500">.</span></span>
        </div>

        <div class="hidden lg:flex items-center space-x-10 text-[11px] font-black uppercase tracking-widest text-slate-400">
          <a href="#features" class="hover:text-orange-500 transition-colors">Solutions</a>
          <a href="#geofence" class="hover:text-orange-500 transition-colors">Geofencing</a>
          <router-link to="/about" class="hover:text-orange-500 transition-colors">About</router-link>
          <a href="#pricing" class="hover:text-orange-500 transition-colors">Pricing</a>
          <router-link to="/login" class="text-slate-900 hover:text-orange-500 transition-colors">Sign In</router-link>
        </div>

        <router-link to="/register-org" class="px-6 py-3 bg-slate-900 text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-orange-500 hover:shadow-xl hover:shadow-orange-200 transition-all active:scale-95">
          Start Free Trial
        </router-link>
      </div>
    </nav>
    ...
    <header class="relative pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden">
      <div class="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
        ...
        <div class="space-y-8 animate-in fade-in slide-in-from-left-8 duration-1000">
          <div class="inline-flex items-center space-x-2 px-4 py-2 bg-orange-50 rounded-full border border-orange-100">
            <span class="relative flex h-2 w-2">
              <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-orange-400 opacity-75"></span>
              <span class="relative inline-flex rounded-full h-2 w-2 bg-orange-500"></span>
            </span>
            <span class="text-[10px] font-black text-orange-500 uppercase tracking-[0.2em]">Live Workforce Intelligence</span>
          </div>

          <h1 class="text-5xl lg:text-7xl font-black text-slate-900 leading-[1.1] tracking-tighter">
            Every second <br />
            <span class="text-orange-500 italic">accounted for.</span>
          </h1>
          ...
          <div class="flex flex-col sm:flex-row gap-4">
            <router-link to="/register-org" class="px-10 py-5 bg-orange-500 text-white rounded-lg font-black text-sm uppercase tracking-widest shadow-2xl shadow-orange-200 hover:bg-orange-600 hover:-translate-y-1 transition-all flex items-center justify-center">
        ...
        <div class="relative animate-in fade-in slide-in-from-right-8 duration-1000 delay-200">
          <div class="relative z-10 bg-slate-900 rounded-xl p-4 shadow-3xl border-[12px] border-slate-800 aspect-video flex items-center justify-center overflow-hidden">
             ...
             <div class="w-full h-full bg-slate-800 rounded-xl p-6 space-y-4">
                ...
                <div class="grid grid-cols-3 gap-4">
                  <div class="h-20 bg-orange-500/20 rounded-xl border border-orange-500/30"></div>
                  <div class="h-20 bg-slate-700 rounded-xl"></div>
                  <div class="h-20 bg-slate-700 rounded-xl"></div>
                </div>
                <div class="h-32 bg-slate-700 rounded-xl"></div>
             </div>
          </div>

          <!-- FLOATING LIVE PULSE (The "Cool" Feature) -->
          <Transition name="notification">
            <div v-if="currentPulse" class="absolute -top-4 -right-4 z-20 bg-white p-4 rounded-xl shadow-2xl border border-slate-100 flex items-center space-x-4 min-w-[240px]">
          ...
          <!-- Background Blobs -->
          <div class="absolute -top-20 -right-20 w-64 h-64 bg-orange-400/20 rounded-full blur-[100px]"></div>
    ...
    <section id="geofence" class="py-24 lg:py-32 bg-[#F8FAFC]">
      <div class="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-20 items-center">
        <div class="order-2 lg:order-1">
            <div class="bg-white p-4 rounded-xl shadow-2xl border border-slate-100 relative">
                ...
                <div class="bg-slate-50 rounded-xl h-[400px] overflow-hidden relative flex items-center justify-center">
                    ...
                    <!-- The Geofence Circle -->
                    <div class="w-64 h-64 rounded-full border-4 border-orange-500 bg-orange-500/10 animate-pulse flex items-center justify-center">
                        <div class="w-2 h-2 bg-orange-500 rounded-full shadow-[0_0_20px_rgba(79,70,229,1)]"></div>
                    </div>
                    <div class="absolute bottom-10 left-10 right-10 bg-white p-4 rounded-xl shadow-xl flex items-center justify-between border border-slate-100">
        ...
        <div class="space-y-8 order-1 lg:order-2">
          <h2 class="text-4xl lg:text-5xl font-black text-slate-900 tracking-tight leading-none">
            Built-in <br /><span class="text-orange-500">Location Intelligence.</span>
          </h2>
          ...
          <ul class="space-y-4">
            <li v-for="item in geoFeatures" :key="item" class="flex items-center space-x-3 text-sm font-bold text-slate-700 uppercase tracking-tight">
              <CheckCircleIcon class="w-5 h-5 text-orange-500" />
              <span>{{ item }}</span>
            </li>
          </ul>
        </div>
    ...
    <section id="features" class="py-24 lg:py-32 bg-white">
      <div class="max-w-7xl mx-auto px-6 text-center mb-20 space-y-4">
        <p class="text-xs font-black text-orange-500 uppercase tracking-[0.4em]">Enterprise Core</p>
      ...
      <div class="max-w-7xl mx-auto px-6 grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        <div v-for="feature in featureCards" :key="feature.title" class="group p-10 rounded-xl bg-slate-50 hover:bg-slate-900 transition-all duration-500 border border-slate-100">
          <div class="w-14 h-14 bg-white rounded-xl flex items-center justify-center mb-8 shadow-sm group-hover:bg-orange-500 transition-colors">
            <component :is="feature.icon" class="w-6 h-6 text-orange-500 group-hover:text-white" />
          </div>
    ...
    <section class="py-20 px-6">
        <div class="max-w-5xl mx-auto bg-slate-900 rounded-xl p-12 lg:p-24 text-center space-y-10 relative overflow-hidden">
            <div class="relative z-10">
                ...
                <div class="flex flex-col sm:flex-row justify-center gap-4 pt-6">
                    <router-link to="/register-org" class="px-10 py-5 bg-orange-500 text-white rounded-xl font-black text-sm uppercase tracking-widest shadow-xl hover:bg-orange-500 transition-all">Start 14-Day Free Trial</router-link>
                    <router-link to="/login" class="px-10 py-5 bg-white/10 text-white rounded-xl font-black text-sm uppercase tracking-widest hover:bg-white/20 transition-all">Sign In</router-link>
                </div>
            </div>
            <!-- Glows -->
            <div class="absolute top-0 right-0 w-64 h-64 bg-orange-500/20 blur-[100px]"></div>
            <div class="absolute bottom-0 left-0 w-64 h-64 bg-orange-500/10 blur-[100px]"></div>
        </div>
    </section>
    ...
    <footer class="py-12 border-t border-slate-100">
      ...
      <div class="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-8">
        ...
        <div class="flex space-x-8 text-[10px] font-black uppercase tracking-widest text-slate-400">
            ...
            <a href="#" class="hover:text-orange-500">Security</a>
            <a href="#" class="hover:text-orange-500">Status</a>
        </div>
