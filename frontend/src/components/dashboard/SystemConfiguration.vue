  ...
  <div class="space-y-8">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 flex flex-wrap gap-2">
      <button
        ...
        :class="[
          'px-6 py-3 rounded-lg font-bold text-sm uppercase tracking-wider transition-all',
          activeTab === tab.id
    ...
    <div v-else-if="activeTab === 'system'" class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-2xl border border-blue-200 shadow-sm p-6">
          <p class="text-xs font-bold text-blue-600 uppercase tracking-wider mb-2">Version</p>
          <p class="text-xl font-black text-blue-600">{{ config.system.version }}</p>
        </div>

        <div class="bg-white rounded-2xl border border-green-200 shadow-sm p-6">
          <p class="text-xs font-bold text-green-600 uppercase tracking-wider mb-2">Environment</p>
          <p class="text-xl font-black text-green-600">{{ config.system.environment }}</p>
        </div>
    ...
    <div v-else-if="activeTab === 'database'" class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-2xl border border-blue-200 shadow-sm p-6">
          <p class="text-xs font-bold text-blue-600 uppercase tracking-wider mb-2">Database Size</p>
          <p class="text-2xl font-black text-blue-600">{{ config.database.sizeInMB }} <span class="text-sm">MB</span></p>
        </div>

        <div class="bg-white rounded-2xl border border-green-200 shadow-sm p-6">
          <p class="text-xs font-bold text-green-600 uppercase tracking-wider mb-2">Backup Status</p>
          <p class="text-xl font-black text-green-600">{{ config.database.backupStatus }}</p>
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
        ...
        <div class="space-y-4">
          <button class="w-full px-6 py-3 bg-primary-500 text-white rounded-lg font-bold hover:bg-primary-600 transition-all text-sm uppercase tracking-wider">
            💾 Create Full Backup Now
          </button>
          <button class="w-full px-6 py-3 bg-slate-200 text-slate-700 rounded-lg font-bold hover:bg-slate-300 transition-all text-sm uppercase tracking-wider">
            📥 Download Latest Backup
          </button>
          <button class="w-full px-6 py-3 bg-slate-200 text-slate-700 rounded-lg font-bold hover:bg-slate-300 transition-all text-sm uppercase tracking-wider">
            🗑️ Prune Old Logs (>90 days)
          </button>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="activeTab === 'security'" class="space-y-6">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div v-for="(value, key) in config.security" :key="key" class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
      ...
      <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
        ...
        <div class="space-y-3">
          ...
          <button class="w-full px-6 py-3 bg-red-500 text-white rounded-lg font-bold hover:bg-red-600 transition-all text-sm uppercase tracking-wider">
            ⚠️ Revoke All Active Sessions
          </button>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="activeTab === 'features'" class="space-y-6">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div
          ...
          :key="key"
          class="bg-white rounded-2xl border shadow-sm p-6 flex items-center justify-between"
          :class="value ? 'border-green-200 bg-green-50' : 'border-slate-200'"
        >
          ...
          <div class="flex items-center gap-2">
            <span
              ...
              :class="value ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-700'"
            >
              {{ value ? '✓ On' : '✗ Off' }}
            </span>
          </div>
    ...
    <div v-else-if="activeTab === 'performance'" class="space-y-6">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div v-for="(value, key) in config.performance" :key="key" class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
          <p class="text-xs font-bold text-slate-600 uppercase tracking-wider mb-4">{{ formatLabel(key) }}</p>
          <div v-if="typeof value === 'boolean'" class="flex items-center gap-3">
    ...
    <div v-else-if="activeTab === 'health'" class="space-y-6">
      ...
      <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
        ...
        <div v-if="diagnostics.checks" class="space-y-4">
          <div
            ...
            :class="status === 'Healthy' ? 'border-green-300 bg-green-50' : 'border-amber-300 bg-amber-50'"
          >
            ...
            <span
              ...
              :class="status === 'Healthy' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'"
            >
              {{ status }}
            </span>
          </div>
      ...
      <!-- Recommendations -->
      <div v-if="diagnostics.recommendations && diagnostics.recommendations.length > 0" class="bg-amber-50 rounded-2xl border border-amber-200 shadow-sm p-8">
      ...
      <!-- Issues -->
      <div v-if="diagnostics.issues && diagnostics.issues.length > 0" class="bg-red-50 rounded-2xl border border-red-200 shadow-sm p-8">
        <h4 class="text-lg font-black text-red-900 mb-4">⚠️ Issues Found</h4>
        <ul class="space-y-2">
