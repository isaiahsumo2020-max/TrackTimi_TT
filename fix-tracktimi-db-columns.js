...
// Connect to the root level tracktimi.db file
const db = new sqlite3.Database('./tracktimi.db', (err) => {
  if (err) {
    console.error('❌ Failed to connect to database:', err.message);
    process.exit(1);
  }
  console.log('✅ Connected to ./tracktimi.db');
});
...
(async () => {
  try {
    await addColumnIfNotExists('Org_ID', 'Org_ID INTEGER');
    await addColumnIfNotExists('Device_ID', 'Device_ID INTEGER');
    await addColumnIfNotExists('IP_Address', 'IP_Address TEXT');
